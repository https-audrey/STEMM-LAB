...
type Nav = StackNavigationProp<RootStackParamList, 'Act7Experiment2'>;
...
const ExperimentPage2: React.FC = () => {
    ...
    // Previous records from Phase 1
    const previousDocIds = route.params?.docIds || [];

    const [isRecording, setIsRecording] = useState(false);
    const [timerValue, setTimerValue] = useState(60); // countdown
            ...
            if (useSimulation) {
                ...
                accelSubscription = Accelerometer.addListener(data => {
                    ...
                    if (accelDataRef.current.length >= 30) {
                        ...
                        let peaks = 0;
                        let lastState = 0; 
                        const threshold = 0.012; // sensitivity
    ...
    const handleRecord = async () => {
        if (isRecordingRef.current) {
            ...
            try {
                const finalRate = respirationRate || 18; // slightly different default
                ...
                Alert.alert(
                    'Experiment 2 Recorded!',
                    `Respiration Rate: ${finalRate} BPM\nDuration: ${duration}s`,
                    [{
                        text: 'OK',
                        onPress: () => {
                            navigation.navigate('Act7Result2', {
    ...
    const handleRestart = () => {
        ...
        accelDataRef.current = [];
        if (timerIntervalRef.current) clearInterval(timerIntervalRef.current);
        Alert.alert('Reset', 'Experiment 2 metrics cleared.');
    };

    const handleContinue = () => {
        navigation.navigate('Act7Result2', { 
            docIds: lastSavedDocIdRef.current 
              ? [...previousDocIds, lastSavedDocIdRef.current] 
              : previousDocIds 
        });
    };
        ...
        <View style={styles.container}>
            ...
            >
                ...
                <View style={styles.boxContainer}>
                    <Image
                        source={require('../../assets/ExperimentAssets/act7ExperimentBox2.png')}
